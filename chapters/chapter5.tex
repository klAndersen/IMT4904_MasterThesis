...
When considering to use stemming, there were two that were considered, Porter and Lancaster. \\
write something more about stemming and choices here \\
surprising that it got worse result \\
combining and not combining related words during stemming
\begin{table}[!h]%[tbp]
	\centering
...
\end{table}
\vspace{0.5em}\newline
In Table \ref{tab:unprocessed_vs_all_feature_detectors_sgd_so}, the classifier results for the \gls{sgd} using all loss functions are omitted because it selected loss='log', giving the same values. 
