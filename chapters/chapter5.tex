...
\todo[inline]{this section is a bit messy, and will be re-written. as of now I'm just focusing on getting most of my notes in, and then removing the non-relevant later on.}
...
Aside from the examples from Scikit-learns own pages \cite{Scikitlearn.org2016j, Scikitlearn.org2016,Scikitlearn.org2016h}, two that helped getting some progress were \cite{Rehurek2014} and \cite{Elahi2016}.
...
Furthermore, the printout I got made no sense (shown in Listing \ref{lst:svc_tutorial}), and I could not understand how he knew that the values he selected were the best.
...
\end{lstlisting}
In addition to having no access to the data sets that were used by \cite{Rehurek2014} and \cite{Elahi2016}, the Scikit-learn version I used was the developer version (0.18.dev0). 
In this version, a lot of the old modules had either been changed or removed, so for those tutorials I did found with a minimum working example, I did not get the same results.

% find a place to put this
In one of Scikit-learns tutorials for text data \cite{Scikitlearn.org2016h}, they have suggested assignments with included solutions. 
...
Furthermore, when looking at the suggested "road map" (see Appendix \ref{app:ml_map}, Figure \ref{fig:ml_map} on p.~\pageref{app:ml_map}), 
I accidentally misread a zero and decided to try \gls{sgd} instead of \gls{svc}.
\vspace{0.5em}\newline
