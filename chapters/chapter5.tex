...
The third option is based on the representation of the sample size, and can be achieved by using quartiles. 
Quartiles is measured by a given percentage of the observations, represented by the three observations. 
Q$_{1}$ (25\%), M (median, 50\%) and Q$_{3}$ (75\%) \cite{Hagen2011}.
Quartiles can be found by using the equation shown in Equation \ref{eq:quartile1} - \ref{eq:quartile3}, and the result for the \gls{so} database is shown in Equation 
\ref{eq:quartile_selection}\footnote{
It should be noted that I have used the first Quartile for both the good and the bad questions, because it represents how many one would need to retrieve for each. 
If you were to retrieve this from the database, then the limit would be the result, with the addition of sorting the results based on the highest or lowest score.	
}.
...
\begin{equation}\label{eq:quartile_selection}
\begin{split}
Q_{1} = \frac{(659,955 + 1)}{4} = 164,989 \\
Q_{1} = \frac{(5,286,971 + 1)}{4} = 1,321,743
\end{split}
\end{equation}
